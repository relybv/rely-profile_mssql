...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_mssql"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/rely-profile_mssql/master/files/bootme.ps1')) -ArgumentList ("profile_mssql")
#</powershell>


  param(
    [string]$role = "profile_mssql"
  )

  $puppet_source = "https://github.com/relybv/rely-profile_mssql.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_mssql" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_mssql" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_mssql"
  }
  ...
  $puppet_module = "giuseppeborgese/disk_init_win"
  $puppet_arg = $puppet_modinst + $puppet_module
  $process = Start-Process -FilePath $puppet_path -ArgumentList $puppet_arg -Wait -PassThru -NoNewWindow
  if ($process.ExitCode -ne 0) {
    Write-Host "Install of $puppet_module failed."
  ...
  $puppet_module = "insentia/windows_sql"
  $puppet_arg = $puppet_modinst + $puppet_module
  $process = Start-Process -FilePath $puppet_path -ArgumentList $puppet_arg -Wait -PassThru -NoNewWindow
  if ($process.ExitCode -ne 0) {
    Write-Host "Install of $puppet_module failed."
  ...
  $puppet_module = "gildas-sqlserver"
  $puppet_arg = $puppet_modinst + $puppet_module
  $process = Start-Process -FilePath $puppet_path -ArgumentList $puppet_arg -Wait -PassThru -NoNewWindow
  if ($process.ExitCode -ne 0) {
    Write-Host "Install of $puppet_module failed."
